...
public class HTTPParser //TODO convert to use new parsing routines and Characters throughout
{
	...
	public static HTTPVersion parseVersion(final CharSequence versionCharSequence) throws SyntaxException {
		...
		if(startsWith(versionCharSequence, VERSION_IDENTIFIER) && versionCharSequence.length() > versionBeginIndex
				&& versionCharSequence.charAt(versionBeginIndex - 1) == VERSION_SEPARATOR) {
	...
	/**
	 * Parses a line of text from a message header, assuming each line ends in CRLF and the content is encoded in the {@value HTTP#HTTP_URI_SCHEME} charset. All
	 * spaces and horizontal tabs are folded into a single space.
	 * @param inputStream The source of the HTTP message.
	...
	*/
	...
	/**
	 * Parses a list of weighted strings (string with an optional qvalue parameter), reading until the end of the reader is reached. If no weight is specified for
	 * a list item, it defaults to 1.0. Any data after a qvalue in each list element is ignored. The returned values are returned in order sorted by weight from
	 * lowest to highest.
	 * @param reader The source of the data.
	 * @throws IOException if there is an error reading the data.
	 */
	...
	/**
	 * Parses a attribute name/value pair from the given reader. Quotes are removed from quoted values.
	 * @param reader The source of the data.
	 * @throws IOException if there is an error reading the data.
	...
	/**
	 * Parses a list of attribute name/value pair from the given reader. Quoted values are
	 * @param reader The source of the data.
	 * @throws IOException if there is an error reading the data.
	 */
	/*TODO fix
	...
	*/
	...
	/**
	 * Parses an HTTP header and returns the authenticate credentials. This method does not allow the wildcard '*' request-URI for the digest URI parameter.
				...
				switch(AuthenticationScheme.valueOf(scheme.toUpperCase())) { //see which type of authentication scheme this is
					case DIGEST: {
						...
						if(nonceCountString.length() != NONCE_COUNT_LENGTH) { //if the nonce count is not of the correct length
							throw new SyntaxException(AUTHORIZATION_HEADER + ' ' + NONCE_COUNT_PARAMETER + " does not have length " + NONCE_COUNT_LENGTH + ".",
									header.toString());
						}
